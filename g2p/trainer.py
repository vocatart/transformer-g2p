...
from g2p.model import TransformerG2P
from g2p.dataset import TSVDataset
from g2p.util import PAD_IDX, UNK_IDX, BOS_IDX, EOS_IDX, get_dedup_tokens
...
class Trainer:
    def __init__(
        ...
        self.plateau_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.optimizer, factor=plateau_factor, patience=plateau_patience, mode="min"
        )

        self.warmup_scheduler = torch.optim.lr_scheduler.LambdaLR(
            self.optimizer,
            lr_lambda=lambda step: (
                min(1.0, step / max(1, warmup_steps)) if warmup_steps > 0 else 1.0
            ),
        )
        ...
        valid_set, train_set = random_split(
            dataset, [valid_set_size, train_set_size], generator=generator
        )
        ...
        def collate_fn(batch):
            ...
            for word_tensor, phoneme_tensor in batch:
                graphemes_with_tokens = torch.cat(
                    [torch.tensor([BOS_IDX]), word_tensor, torch.tensor([EOS_IDX])]
                )
                grapheme_batch.append(graphemes_with_tokens)

                phonemes_with_tokens = torch.cat(
                    [torch.tensor([BOS_IDX]), phoneme_tensor, torch.tensor([EOS_IDX])]
                )
                phoneme_batch.append(phonemes_with_tokens)

            grapheme_lengths = torch.tensor(
                [len(x) for x in grapheme_batch], dtype=torch.long
            )
            phoneme_lengths = torch.tensor(
                [len(x) for x in phoneme_batch], dtype=torch.long
            )

            grapheme_batch = torch.nn.utils.rnn.pad_sequence(
                grapheme_batch, batch_first=True, padding_value=PAD_IDX
            )
            phoneme_batch = torch.nn.utils.rnn.pad_sequence(
                phoneme_batch, batch_first=True, padding_value=PAD_IDX
            )
    ...
    def _train_epoch(
        self, step: int, total_loss: float, epoch: int, writer: SummaryWriter
    ):
        self.model.train()
        ...
        batch_count = 0
        pbar = tqdm(
            self.training_loader, desc=f"Epoch {epoch} | Step {step}", unit="step"
        )

        for (grapheme_batch, grapheme_lengths), (
            phoneme_batch,
            phoneme_lengths,
        ) in pbar:
            ...
            warmup_status = (
                f"Warmup: {step}/{self.warmup_steps}"
                if step <= self.warmup_steps
                else None
            )
            pbar.set_postfix(
    ...
    def _save_checkpoint(self, step: int, epoch: int, total_loss: float):
        ...
        torch.save(checkpoint, f"{self.artifacts_dir}/model-latest.pt")
    ...
    def train(self):
        ...
        try:
            while step < self.max_steps and not early_stop:
                epoch += 1
                step, total_loss, early_stop = self._train_epoch(
                    step, total_loss, epoch, writer
                )

                writer.add_scalar(
                    "loss/epoch", total_loss / step if step > 0 else 0, epoch
                )
